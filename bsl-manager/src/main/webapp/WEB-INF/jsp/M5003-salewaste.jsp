...
<div>
	<div class="easyui-panel" title="废品发货信息查询" style="padding:10px 10px 10px 10px">
		<form id="saleWasteM5003Form" class="itemForm" method="post">
		   <table>
		        <tr>
		        	<td width="120" align="right">销售通知单号:</td>
		            <td width="210" align="right">
		            	<input name="salePlanId" id="salePlanIdM5003saleWaste" class="easyui-textbox" type="text" data-options="required:false,validType:'length[0,20]'" style="width:200px;"></input>
		            </td>   
		        </tr>	    
		    </table>
		</form>
		<div style="margin-top:10px" align="center">
	    	<a href="javascript:void(0)" class="easyui-linkbutton" onclick="searchM5003saleWasteForm()">查询</a>
	    	<a href="javascript:void(0)" class="easyui-linkbutton" onclick="clearM5003saleWasteForm()">关闭</a>
		</div>
	</div>
	 <table class="easyui-datagrid" id="saleWasteM5003List" title="废品接收信息管理"  style="height:440px"
	       data-options="singleSelect:false,rownumbers:true,collapsible:true,method:'post',url:'/saleProdByPlan/waitSaleWassteGroups',toolbar:toolbarM5003saleWaste">
	    <thead>
	        <tr>
...
<script>
	...
	//查询按钮
	function searchM5003saleWasteForm(){
		//ajax的post方式提交表单
		//$("#saleDetailSearchForm").serialize()将表单序列号为key-value形式的字符串
		$.post("/saleProdByPlan/waitSaleWassteGroups",$("#saleWasteM5003Form").serialize(), function(data){
			if(data.status == 200){
	...
	//关闭按钮
	function clearM5003saleWasteForm(){
		$("#saleWasteWindowM5003").window('close');
	}
	
    function getM5003saleWasteSelectionsIds(){
    	var saleDetailList = $("#saleWasteM5003List");
    ...
    var toolbarM5003saleWaste = [{
        text:'确认发货',
        iconCls:'icon-ok',
        handler:function(){
        	var ids = getM5003saleWasteSelectionsIds();
        	if(ids.length == 0){
        	...
        	$.messager.confirm('确认','确认选择这些记录发货？',function(r){
				if (r){
        	    	...
        	    	var params = {'arrays':strArrays,"prodOutPlan":$('#salePlanIdM5003saleWaste').textbox("getValue"),"checkUser":$("#user_id").html()};
        	   			...
        	   			if(data.status == 200){
        	   				$.messager.alert('提示','发货成功!',undefined,function(){
        	   					...
        	   					$.messager.confirm('确认','是否需要打印本次发货信息？',function(r){
        	   						if (r){
        	   							var bsId = $("#salePlanIdM5003saleWaste").textbox('getValue');
        	   							var prodIds = [];
