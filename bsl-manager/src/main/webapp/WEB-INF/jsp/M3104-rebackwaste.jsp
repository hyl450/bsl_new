...
<div style="pEditing:10px 10px 10px 10px">
	<form id="jsRebackWasteForm" class="itemForm" method="post">
	   <table>
	   		<tr>
	        	<td width="120" align="right">销售计划流水:</td>
	            <td width="210" align="right">
	            	<input name="saleSerno" id="saleSernoM3104rebackwasteOut" class="easyui-textbox" type="text" readonly="readonly" data-options="required:true,validType:'length[0,20]'" style="width:200px;"></input>
	            </td>
	       ...
	       <tr> 
	        ...
	        <tr> 
	        	...
	        	<td width="120" align="right">本次接收退回重量/吨:</td>
	            <td width="210" align="right">
	            	<input name="prodOutWeight" id="prodM3104rebackwaste" class="easyui-numberbox" type="text"  data-options="required:false,min:0,precision:3,validType:'length[0,10]'" style="width:200px;"></input>
	            </td>
	            <td width="120" align="left">
	        		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="getWeightCheckM3104rebackwasteout()" style="width:70px;">称重</a>
	        	</td>
	        </tr>
	        <tr>
	...
	<div style="margin-top:10px" align="center">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitM3104rebackwasteOutForm()">提交</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="closeM3104rebackwasteOutForm()">关闭</a>
	</div>
</div>
<script type="text/javascript">

	//联动称重机获取实际重量
	function getWeightCheckM3104rebackwasteout(){
		var params = {};
		$.post("/rxtx/weighing",params, function(data){
			if(data.status == 200){
				$("#prodM3104rebackwaste").numberbox('setValue',data.data);
			}else{
	...
	//提交表单
	function submitM3104rebackwasteOutForm(){
		//有效性验证
		if(!$('#jsRebackWasteForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return;
		}
		//记录录入人员信息
		var inputUser = $("#user_id").html();
		var prodOutWeight =  $("#prodM3104rebackwaste").numberbox('getValue');
		var saleSerno = $("#saleSernoM3104rebackwasteOut").textbox('getValue');
		...
		$.post("/prodWxJs/rebackWaste",params, function(data){
			if(data.status == 200){
				$.messager.alert('提示','废品接收退回成功!单号为：'+data.data,'info',function(){
					$("#jsWasteRebackWindow").window('close');
	...
	function closeM3104rebackwasteOutForm(){
		$("#jsWasteRebackWindow").window('close');
	}
