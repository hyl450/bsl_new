...
<div style="pEditing:10px 10px 10px 10px">
	<form id="saleWasteWxDetailRebackForm" class="itemForm" method="post">
	   <table>
	   		<tr>
	        	<td width="120" align="right">销售计划流水:</td>
	            <td width="210" align="right">
	            	<input name="saleSerno" id="saleSernoM5005WxReback" class="easyui-textbox" type="text" readonly="readonly" data-options="required:true,validType:'length[0,20]'" style="width:200px;"></input>
	            </td>
	       ...
	       <tr> 
	        ...
	        <tr>
	            ...
	            <td width="210" align="right">
	            	<input name="prodRebackWeight" id="prodRebackWeightRebackM5005Wx" class="easyui-numberbox" type="text" data-options="required:false,min:0,precision:3,validType:'length[0,10]'" style="width:200px;"></input>
	            </td>
	            <td width="120" align="left">
	        		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="getWeightCheckM5005WxReback()" style="width:70px;">称重</a>
	        	</td>
	        </tr>
	    </table>
	</form>
	<div style="margin-top:10px" align="center">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitM5005WxRebackForm()">提交</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="closeM5005WxRebackForm()">关闭</a>
	</div>
</div>
<script type="text/javascript">

	//联动称重机获取实际重量
	function getWeightCheckM5005WxReback(){
		var params = {};
		$.post("/rxtx/weighing",params, function(data){
			if(data.status == 200){
				$("#prodRebackWeightRebackM5005Wx").numberbox('setValue',data.data);
			}else{
	...
	//提交表单
	function submitM5005WxRebackForm(){
		//有效性验证
		if(!$('#saleWasteWxDetailRebackForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return;
		}
		//记录录入人员信息
		var inputUser = $("#user_id").html();
		var prodRebackWeight =  $("#prodRebackWeightRebackM5005Wx").numberbox('getValue');
		var saleSerno = $("#saleSernoM5005WxReback").textbox('getValue');
		...
		$.post("/saleWasteWx/reback",params, function(data){
			if(data.status == 200){
				$.messager.alert('提示','废品入库成功!单号为：'+data.data,'info',function(){
					$("#saleWasteWxDetailRebackWindow").window('close');
					searchM5005wForm();
	...
	function closeM5005WxRebackForm(){
		$("#saleWasteWxDetailRebackWindow").window('close');
	}
