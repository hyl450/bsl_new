...
<div style="padding:10px 10px 10px 10px">
	<form id="enumEditForm" class="itemForm" method="post">
	   <table>
	        <tr>
	        ...
	        <tr>
	            ...
	            <td width="210" align="right">
	            	<input name="enumKey" class="easyui-textbox" type="text" readonly="readonly" data-options="required:true,validType:'length[0,20]'" style="width:200px;"></input>
	            </td>
	...
	<div style="margin-top:10px" align="center">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitEditM0004Form()">提交</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="closeEditM0004Form()">关闭</a>
	</div>
</div>
<script type="text/javascript">
	
	//提交表单
	function submitEditM0004Form(){
		
		//有效性验证
		if(!$('#enumEditForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return;
		}
		//ajax的post方式提交表单
		//$("#itemAddForm").serialize()将表单序列号为key-value形式的字符串
		$.post("/enum/edit",$("#enumEditForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','修改数据字典项成功!','info',function(){
					$("#enumEditWindow").window('close');
					searchM0004Form();
				});
			}else{
