...
<div style="padding:10px 10px 10px 10px">
	<form id="wasteCheckForm" class="itemForm" method="post">
	   <table>
	        <tr>
	            ...
	            <td width="210" align="right">
	            	<input name="wasteWeight" id="wasteWeightM4001Check" class="easyui-numberbox" type="text" data-options="required:true,min:0,precision:3,validType:'length[0,10]'"  style="width:200px;"></input>
	            </td>
	            <td width="120" align="left">
	        		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="getWeightM4001Check()" style="width:70px;">称重</a>
	        	</td>  
	        </tr>
	        <tr>	
	        ...
	        <tr>	
	         ...
	         <tr hidden="true">
	        	...
	        	<td width="120" align="right">录入人:</td>
	        	 <td width="210"  align="right">
	        		<input name="inputUser" id="inputuserM4001Check" class="easyui-textbox" type="text" data-options="required:false" style="width:200px;"></input>
	        	</td>
	        </tr>
	    </table>
	</form>
	<div style="margin-top:10px" align="center">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitM4001CheckForm()">提交</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="closeM4001CheckForm()">关闭</a>
	</div>
</div>
<script type="text/javascript">
	
	//联动称重机获取实际重量
	function getWeightM4001Check(){
		var params = {};
		$.post("/rxtx/weighingByD",params, function(data){
			if(data.status == 200){
				$("#wasteWeightM4001Check").numberbox('setValue',data.data);
			}else{
	...
	//提交表单
	function submitM4001CheckForm(){
		
		//有效性验证
		if(!$('#wasteCheckForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return;
		}
		var inputUser = $("#user_id").html();
		$("#inputuserM4001Check").textbox('setValue',inputUser);
		//ajax的post方式提交表单
		//$("#itemAddForm").serialize()将表单序列号为key-value形式的字符串
		$.post("/waste/check",$("#wasteCheckForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','废品重量修改成功!实际重量为：'+data.data,'info',function(){
					$("#wasteCheckWindow").window('close');
					searchM4001Form();
