        ...
        try {  
            ...
            for(int i=0; i<sheetNum; i++){  
                ...
                if(1==sheetNum){  
                    ...
                    fillSheet(sheet, list, sheetName, fieldMap, 0, list.size()-1);  
                ...
                //有多个工作表的情况  
                    ...
                    //填充工作表  
                    fillSheet(sheet, list, sheetName, fieldMap, firstIndex, lastIndex);  
                }  
            }  
...
//        String fileName=new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()).toString();  
    ...
    public static <T>  void   listToExcel (  
        ...
        listToExcel(list, fieldMap, sheetName, excelName, 65535, response);  
    }  
	...
	//        wcf.setVerticalAlignment(jxl.format.VerticalAlignment.CENTRE);//把垂直对齐方式指定为居中
	        //填充业务名称和导出时间
	        sheet.addCell(new Label(0,0,"业务名称："+sheetName));
	        //合并第一列第一行到第cnFields.length列第一行的所有单元格
	        sheet.mergeCells(0,0,cnFields.length-1,0);
	        sheet.addCell(new Label(0,1,"导出日期："+simpleDateFormat.format(new Date())));
	        //合并第一列第二行到第cnFields.length列第二行的所有单元格
	        sheet.mergeCells(0,1,cnFields.length-1,1);
	        //填充表头  
	        for(int i=0;i<cnFields.length;i++){  
	            Label label=new Label(i,2,cnFields[i]);  
	            sheet.addCell(label);  
	        }  
	            ...
	            for(int i=0;i<enFields.length;i++){  
	                ...
	                if(objValue != null) {
	                	if(objValue instanceof Date) {
			    			...
			    			String value = jedisClient.get(enFields[i]+":"+objValue.toString());
			    			fieldValue=value==null ? objValue.toString() : value;  
	        			}
	                }
	 ...
	 * @return
	 */
	public static String getExcelName(String excelName,String loginUserId) {
		return excelName+"_"+sdfFull.format(new Date())+"_"+loginUserId+".xls";
	}
